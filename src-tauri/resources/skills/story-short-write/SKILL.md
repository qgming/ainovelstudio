...
- 短篇集工厂索引（仅短篇集工厂场景由发布整理流程维护；单篇项目通常不存在，读不到则跳过避重即可，不要主动创建）。用于避开最近几篇已用过的题材 / 关系 / 反转 / 结局。
...
- **避重**：与短篇集工厂索引中最近几篇的题材 / 关系 / 反转 / 结局有明显差异。

## Outputs / Write-Back

- 当前短篇项目目录及其子文件（立项 / 蓝图 / 正文 / 最小设定）。
- `.project/memory/` 的 `type: project` 记忆：本项目的题材、字数、阶段（更新时同步 frontmatter 的 `updated`）。

不要写：短篇集工厂索引（由发布整理子任务登记）；不要回头改其他短篇项目。
