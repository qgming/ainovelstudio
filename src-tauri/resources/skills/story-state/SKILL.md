---
name: story-state
description: 在小说项目存在动态状态时优先使用。它负责在项目工作区中创建、读取、更新和校验 `latest-plot.json`、`character-state.json`、`system-state.json`、`continuity-index.json` 一类结构化状态文件，把正文新增信息同步为可程序化维护的动态真值层，供 `story-writer`、`continuity-check`、`story-bible` 和 `outline-manager` 读取。
---
...
2. 在工作区中寻找已有 `状态/`、`state/` 或其他 JSON 状态目录；如果没有，创建最小必要结构。
