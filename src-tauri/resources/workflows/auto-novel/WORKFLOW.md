...
5. 审查未通过时，由条件节点判断是否允许返工；通过时进入设定更新
...
- 同一章节返工时，写作节点只能修订当前章节，不能误写下一章
- 返工上限和主循环上限由工作流配置控制，而不是由代理自行决定；次数支持有限值或无限
...
- 左侧基础区可直接设置最大循环次数、每轮最大返工次数，并支持切换为无限
