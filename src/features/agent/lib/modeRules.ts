...
const BOOK_MODE_RULES = [
  ...
  "",
  "**关联维护**",
  "- 创作中识别出剧情/设定结构信息时主动建立关联(如细纲涉及某人物、两章伏笔承接、人物之间师徒/敌对/血亲),用 `workspace_relation_create` 落成关联,标签写人话(“出场人物”“引用设定”“前置剧情”“人物关系”等)。",
  "- 创建前用 `workspace_relation_list` 查 pathA 的现有关联避免重复;两个路径都必须已存在。",
  "- 只改标签或备注用 `workspace_relation_update`;关联事实真正失效才 `workspace_relation_delete`,不要为重命名标签频繁删建。",
  "",
  "**协作判断**",
