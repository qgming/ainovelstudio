...
vi.mock("@features/update/api/updateApi", () => ({
...
describe("updateStore", () => {
  ...
  it("检测到新版本后会保存更新摘要", async () => {
    fetchLatestDirectUpdateMock.mockResolvedValue({
      downloadUrl: "https://example.com/ainovelstudio_0.5.1_windows_x64.exe",
      notes: "修复若干问题",
      packageKind: "exe",
  ...
  it("下载更新会直接打开外部安装包链接", async () => {
    useUpdateStore.setState({
      ...
      updateSummary: {
        ...
        downloadUrl: "https://example.com/ainovelstudio_0.2.8_windows_x64.exe",
        notes: "修复若干问题",
        packageKind: "exe",
        publishedAt: "2026-04-21T00:00:00Z",
        version: "0.2.8",
    ...
    expect(openExternalUpdateUrlMock).toHaveBeenCalledWith(
      "https://example.com/ainovelstudio_0.2.8_windows_x64.exe",
    );
  ...
  it("启动时静默检查更新并保留可用版本", async () => {
    ...
    expect(useUpdateStore.getState().status).toBe("available");
    expect(useUpdateStore.getState().updateSummary?.version).toBe("0.5.1");
