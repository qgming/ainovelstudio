...
function renderBookWorkspaceView(props: React.ComponentProps<typeof BookWorkspaceView> = {}) {
  return render(
    <TooltipProvider>
      <BookWorkspaceView {...props} />
    </TooltipProvider>,
  );
}
...
describe("BookWorkspaceView", () => {
  ...
  it("已打开书籍后左上角改为返回首页按钮，不再打开书籍菜单", async () => {
    ...
    renderBookWorkspaceView({ onNavigateHome: handleNavigateHome });
  ...
  it("按请求的 bookId 打开时，不会先把路由回写成旧书", async () => {
    ...
    renderBookWorkspaceView({ requestedBookId: otherBookId, onWorkspaceBookChange: handleWorkspaceBookChange });
  ...
  it("页面卸载时会保留正在运行的 Agent 会话", () => {
    ...
    const view = renderBookWorkspaceView();
    view.unmount();
