...
describe("AboutSection", () => {
  ...
  it("检测到新版本后会直接展示更新日志弹窗", () => {
    ...
    updateStoreState.updateSummary = { version: "0.2.4" };

    render(<AboutSection />);

    expect(screen.getByRole("dialog")).toBeInTheDocument();
    expect(screen.getByText("发现 0.2.4")).toBeInTheDocument();
  });
  ...
  it("有更新时会弹出更新日志对话框，并通过按钮触发下载", () => {
    ...
    expect(screen.getByRole("dialog")).toBeInTheDocument();
    expect(screen.getByText("发现 0.2.4")).toBeInTheDocument();
    expect(screen.getByText(/修复更新流程[\s\S]*补齐用量统计/)).toBeInTheDocument();
