...
Agent 可以读取当前书籍结构、指定文件、启用的技能和项目规则。它能用 `workspace_search` 检索工作区事实源和正文证据，用 `workspace_read`、`workspace_edit`、`workspace_write` 处理工作区内容，用 `text_stats` 统计字数，也能通过 `update_plan` 维护任务计划。
...
- [v0.2.4](docs/releases/v0.2.4.md)：模型推理开关默认安全值、会话上下文面板等改进。
- [历史版本](docs/releases)
...
- 需要 Windows 桌面端与 Android 安装包的个人创作流程。
